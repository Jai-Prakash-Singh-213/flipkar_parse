...
def addin_to_mysql():
    ...
    cursor = db.cursor()
    cursor.execute( "create table if not exists handbag_brands_info ( \
    ...
    for line in f:
        ...
        sql = """insert into handbag_brands_info(date, position, brand_name, brand_count, brand_link) values ("%s", "%s", "%s", "%s", "%s")"""%(value)
        cursor.execute(sql)
        db.commit()
